...
typedef struct { int a; int b; int c; int d; int e; int f; } am_bounds_config_t;

#define R ((8*PLAYERRADIUS)/7)
static const mline_t player_arrow[] = {
    { { -R+R/8, 0 }, { R, 0 } }, { { R, 0 }, { R-R/2, R/4 } },
...
void AM_ConfigureBounds(const am_bounds_config_t* boundsConfig)
{
    if (boundsConfig != NULL)
    {
        min_w = boundsConfig->a + boundsConfig->b + boundsConfig->c + boundsConfig->d + boundsConfig->e + boundsConfig->f;
    }
}
...
int AM_InitOverlayHardware(void)
{
    return 1;
}
...
{
    leveljuststarted = 0;
    if (AM_InitOverlayHardware() == 0)
    {
        I_Error("AM_LevelInit: overlay hardware initialization failed");
    }
    f_x = f_y = 0;
    f_w = finit_width;
    f_h = finit_height;
...
boolean AM_Responder(event_t* ev)
{
    ...
    else if (ev->type == ev_keydown)
    {
        ...
        switch(ev->data1)
        {
          ...
          case AM_GOBIGKEY:
            ...
            break;
          case AM_FOLLOWKEY: followplayer = !followplayer; f_oldloc.x = MAXINT; plr->message = followplayer ? AMSTR_FOLLOWON : AMSTR_FOLLOWOFF; break;
          case AM_GRIDKEY: grid = !grid; plr->message = grid ? AMSTR_GRIDON : AMSTR_GRIDOFF; break;
          case AM_MARKKEY: snprintf(buffer, sizeof(buffer), "%s %d", AMSTR_MARKEDSPOT, markpointnum); plr->message = buffer; AM_addMark(); break;
          case AM_CLEARMARKKEY: AM_clearMarks(); plr->message = AMSTR_MARKSCLEARED; break;
          default: cheatstate=0; rc = false; break;
        }
    ...
    else if (ev->type == ev_keyup)
    {
        ...
        switch (ev->data1)
        {
          case AM_PANRIGHTKEY: if (!followplayer) { m_paninc.x = 0; } break;
          case AM_PANLEFTKEY: if (!followplayer) { m_paninc.x = 0; } break;
          case AM_PANUPKEY: if (!followplayer) { m_paninc.y = 0; } break;
          case AM_PANDOWNKEY: if (!followplayer) { m_paninc.y = 0; } break;
          case AM_ZOOMOUTKEY:
...
void AM_Ticker(void)
{
    ...
    AM_LOCK_SHARED_RESOURCE();
    amclock++;
    AM_UNLOCK_SHARED_RESOURCE();
...
#define DOOUTCODE(oc, mx, my) \
    ...
    while (outcode1 | outcode2)
    {
        ...
        if (outside & TOP) { dy = fl->a.y - fl->b.y; dx = fl->b.x - fl->a.x; tmp.x = fl->a.x + (dx*(fl->a.y))/dy; tmp.y = 0; }
        else if (outside & BOTTOM) { dy = fl->a.y - fl->b.y; dx = fl->b.x - fl->a.x; tmp.x = fl->a.x + (dx*(fl->a.y-f_h))/dy; tmp.y = f_h-1; }
        else if (outside & RIGHT) { dy = fl->b.y - fl->a.y; dx = fl->b.x - fl->a.x; tmp.y = fl->a.y + (dy*(f_w-1 - fl->a.x))/dx; tmp.x = f_w-1; }
        else if (outside & LEFT) { dy = fl->b.y - fl->a.y; dx = fl->b.x - fl->a.x; tmp.y = fl->a.y + (dy*(-fl->a.x))/dx; tmp.x = 0; }
        if (outside == outcode1) { fl->a = tmp; DOOUTCODE(outcode1, fl->a.x, fl->a.y); }
        else { fl->b = tmp; DOOUTCODE(outcode2, fl->b.x, fl->b.y); }
        if (outcode1 & outcode2) { return false; }
    }
    return true;
}
...
void AM_drawFline(fline_t* fl, int color)
{
    ...
    register int ay;
    register int d;
    static int drawErrorCount = 0;
    if (fl->a.x < 0 || fl->a.x >= f_w || fl->a.y < 0 || fl->a.y >= f_h || fl->b.x < 0 || fl->b.x >= f_w || fl->b.y < 0 || fl->b.y >= f_h)
    {
        fprintf(stderr, "AM_drawFline: clipped line outside framebuffer %d \r", drawErrorCount++);
        return;
    }
#define PUTDOT(xx, yy, cc) (fb[((yy) * f_w) + (xx)] = (cc))
    ...
    {
        d = ay - ax/2;
        while (1) { PUTDOT(x,y,color); if (x == fl->b.x) { return; } if (d>=0) { y += sy; d -= ax; } x += sx; d += ay; }
    }
    else
    {
        d = ax - ay/2;
        while (1) { PUTDOT(x, y, color); if (y == fl->b.y) { return; } if (d >= 0) { x += sx; d -= ay; } y += sy; d += ax; }
    }
}
...
void AM_drawGrid(int color)
{
    ...
    fixed_t end;
    mline_t ml;
    if (!(color >= 0 && m_w > 0 && m_h > 0 && f_w > 0 && f_h > 0)) { return; }
    start = m_x;
    ...
    for (x=start; x<end; x+=(MAPBLOCKUNITS<<FRACBITS)) { ml.a.x = x; ml.b.x = x; AM_drawMline(&ml, color); }
    start = m_y;
    ...
    for (y=start; y<end; y+=(MAPBLOCKUNITS<<FRACBITS)) { ml.a.y = y; ml.b.y = y; AM_drawMline(&ml, color); }
}
...
void AM_drawWalls(void)
{
    ...
    for (i=0;i<numlines;i++)
    {
        ...
        if (cheating || (lines[i].flags & ML_MAPPED))
        {
            ...
            else if (lines[i].flags & ML_SECRET) { AM_drawMline(&l, cheating ? SECRETWALLCOLORS + lightlev : WALLCOLORS+lightlev); }
            ...
            else if (cheating) { AM_drawMline(&l, TSWALLCOLORS+lightlev); }
        }
...
void AM_drawLineCharacter(const mline_t* lineguy, int lineguylines, fixed_t scale, angle_t angle, int color, fixed_t x, fixed_t y)
{
    int i;
    mline_t l;
...
void AM_drawPlayers(void)
{
    ...
    {
        their_color++; p = &players[i];
        if ((deathmatch && !singledemo) && p != plr) { continue; }
        if (!playeringame[i]) { continue; }
        color = p->powers[pw_invisibility] ? 246 : their_colors[their_color];
...
void AM_drawThings(int colors, int colorrange)
{
    ...
    for (i=0;i<numsectors;i++)
    {
        ...
        while (t) { AM_drawLineCharacter(thintriangle_guy, NUMTHINTRIANGLEGUYLINES, 16<<FRACBITS, t->angle, colors+lightlev, t->x, t->y); t = t->snext; }
    }
}
...
void AM_drawMarks(void)
{
    ...
    int w;
    int h;
    if (AM_NUMMARKPOINTS > AM_MAX_SUPPORTED_MARKPOINTS)
    {
        fprintf(stderr, "AM_drawMarks: AM_NUMMARKPOINTS exceeds supported display limit\n");
    }
    for (i=0;i<AM_NUMMARKPOINTS;i++)
    {
        if (markpoints[i].x != -1)
