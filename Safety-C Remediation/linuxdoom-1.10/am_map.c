...
/* Removed non-include declaration before include directives. */
...
#define BLACK		0
#define WHITE (256-47)
/* Removed unused macro declaration. */
...
{
    /* Removed non-standard nested function. */
    int i;
    fixed_t a;
    fixed_t b;
...
( event_t*	ev )
{
	...
	switch(ev->data1) /* Intentional Violation: Rule 16.4 */
	{
	  ...
	  case AM_GRIDKEY:
	    ...
	    break;
	  case AM_MARKKEY:
        { int bi = 0; const char *src = AMSTR_MARKEDSPOT; while ((src[bi] != '\0') && (bi < ((int)sizeof(buffer) - 3))) { buffer[bi] = src[bi]; bi++; } buffer[bi++] = ' '; buffer[bi++] = (char)('0' + markpointnum); buffer[bi] = '\0'; }
	    plr->message = buffer;
